...
export default function Upload() {
  ...
  const handleFolderUpload = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of fileArray) {
      try {
        ...
        // The first folder is the champion name (id)
    ...
    if (Object.keys(groups).length === 0) {
      ...
      setMessage("Folder structure not detected. Please use the 'Upload Champion Folders' method below.");
    } else {
  ...
  const handleZipUpload = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.name.endsWith('.zip')) {
      setMessage("Please upload a ZIP file containing champion folders.");
      return;
    }
    ...
    try {
      ...
      if (result.groups && Object.keys(result.groups).length > 0) {
        ...
        setMessage(`Successfully processed ${Object.keys(newGroups).length} champion folders.`);
      } else {
        setMessage("No valid champion folders found in the ZIP file.");
      }
    } catch (error) {
  ...
  const handleChampionFolderSelect = (championName: string, files: FileList | null) => {
    ...
    if (!newGroups[championName]) {
      newGroups[championName] = { files: [], previews: [] };
    }

    // Add each file to the champion's group (instead of replacing)
    Array.from(files).forEach(file => {
      if (/\.(jpe?g|png|gif|webp)$/i.test(file.name)) {
        newGroups[championName].files.push(file);
        newGroups[championName].previews.push(URL.createObjectURL(file));
      }
    });
  ...
  // Add a new champion input section
  const addChampionInput = () => {
    const championName = prompt("Enter champion name:");
    if (!championName) return;

    // Create empty group for this champion if it doesn't exist
    if (!imageGroups[championName]) {
      setImageGroups({
        ...imageGroups,
        [championName]: { files: [], previews: [] }
      });
    }
  };
  ...
  const uploadToR2 = async () => {
    ...
    try {
      ...
      for (const championId of Object.keys(imageGroups)) {
        urlsById[championId] = {};
        const files = imageGroups[championId].files;

        // If we don't have actual files (from ZIP processing), skip
        if (files.length === 0 && imageGroups[championId].previews.length > 0) {
        ...
        for (const file of files) {
          ...
          // Store the URL using the filename as key
          urlsById[championId][file.name] = result.url;
        }
      }
  ...
  // Remove a specific image from a champion group
  const removeImage = (championId: string, imageIndex: number) => {
    const newGroups = { ...imageGroups };

    // If the group doesn't exist, return
    if (!newGroups[championId]) return;

    // Revoke the object URL to prevent memory leaks
    const previewUrl = newGroups[championId].previews[imageIndex];
    if (previewUrl.startsWith('blob:')) {
      URL.revokeObjectURL(previewUrl);
    }

    // Remove the file and preview from their respective arrays
    newGroups[championId].files.splice(imageIndex, 1);
    newGroups[championId].previews.splice(imageIndex, 1);

    // Update state
    setImageGroups(newGroups);
  };

  // Remove a champion group
  const removeChampionGroup = (championId: string) => {
    const newGroups = { ...imageGroups };

    // Revoke object URLs for this group
    newGroups[championId].previews.forEach(preview => {
      if (preview.startsWith('blob:')) {
        URL.revokeObjectURL(preview);
      }
    });
    ...
    // Remove the group
    delete newGroups[championId];
    setImageGroups(newGroups);
  };

  // Get sorted champion IDs for display
  const getSortedChampionIds = (): string[] => {
  ...
  return (
    <div className="relative z-10 w-full flex flex-col items-center" ref={topPageRef}>
      <div className="w-full max-w-4xl p-6 bg-white/10 backdrop-blur-md rounded-lg shadow-lg">
        <h1 className="text-2xl mb-6 text-white">Champion Images Upload</h1>
        ...
        <div className="mb-8 p-4 border border-gray-700 rounded-lg">
          <h2 className="text-lg text-white mb-3">Method 1: Upload Champion Folders</h2>
          <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
            ...
            />
            <p className="text-gray-300 text-sm italic">Select the folder containing all champion folders</p>
          </div>
        </div>

        {/* Method 2: Manual Champion Selection */}
        <div className="mb-8 p-4 border border-gray-700 rounded-lg">
          <h2 className="text-lg text-white mb-3">Method 2: Select Champion Images Manually</h2>

          {/* Add Champion button moved to top */}
          <div className="mb-4">
            <button
              onClick={addChampionInput}
          ...
          {/* Existing champion inputs - alphabetically sorted */}
          {getSortedChampionIds().map((championId) => (
            <div key={championId} className="mb-4 p-3 bg-gray-800/50 rounded-lg">
              <div className="flex justify-between items-center mb-2">
                <h3 className="text-white font-medium">{championId}</h3>
                <button
                  onClick={() => removeChampionGroup(championId)}
              ...
              <input
                ...
                multiple
                accept="image/*"
                onChange={(e) => handleChampionFolderSelect(championId, e.target.files)}
                disabled={isLoading}
                className="hidden"
                id={`file-input-${championId}`}
              />
              <label htmlFor={`file-input-${championId}`} className="text-white mr-2 py-1 px-3 rounded-md border-0 text-sm bg-blue-600 text-white hover:bg-blue-700"
              >Add Files</label>


              {/* Preview thumbnails */}
              {/* Preview thumbnails */}
              {imageGroups[championId].previews.length > 0 && (
                <div className="grid grid-cols-4 gap-2 mt-2">
                  {imageGroups[championId].previews.map((preview, idx) => (
                    <div key={idx} className="relative h-16 bg-gray-900 rounded overflow-hidden group">
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img
                        src={preview}
                        alt={`${championId} preview ${idx + 1}`}
                        className="object-contain w-full h-full"
                      />
                      <button
                        onClick={() => removeImage(championId, idx)}
