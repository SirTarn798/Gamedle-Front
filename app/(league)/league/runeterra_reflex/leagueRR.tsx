...
export default function RuneterraReflexCanvas({ username }: Props) {
    ...
    socket.on("gameResult", (data) => {
        toast(
            Toast({
                title: "Game Result",
                text: `${data.place === 1 ? "💪🤴" : data.place === 2 ? "🤏✨" : "💀😭"
                    } You got ${data.place === 1 ? "1st" : data.place === 2 ? "2nd" : "3rd"
                    } place.`,
                bg:
                    data.place === 1
                        ? "bg-lime-400"
                            ...
                            {Object.entries(room?.players || {}).map(([id, player]) => (
                                ...
                                >
                                    ...
                                    >
                                        {player.username}{" "}
                                        <span
                                            className={`font-medium ${player.status === "disconnected"
                                                    ? "text-stone-400"
                                                    : player.status === "alive"
                                                        ? "text-green-400"
                                                        : "text-red-500"
